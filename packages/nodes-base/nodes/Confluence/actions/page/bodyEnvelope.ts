...
export type ConfluenceBodyFormat = 'plainText' | 'storage' | 'adf';
...
export function bodyProperties(operations: string[]): INodeProperties[] {
	...
	return [
		...
		},
		{
			...
			displayOptions: { show: { ...show, bodyFormat: ['adf'] } },
		},
	];
}
...
export function readBodyEnvelope(
	...
	const content = ctx.getNodeParameter(fieldByFormat[format] ?? 'bodyPlainText', itemIndex, '');
	try {
